...
from models import User, Question, Lesson, Section, Progress, DailyTask
...
async def answer_question(db: db_dependency, user: user_dependency, request: AnswerQuestionRequest):
    ...
    questions_per_subsection = max(1, progress.total_questions // 3) # subsection tamamlama kısmı
    ...
    if progress.subsection_completion == 3: # section tamamlandıysa bir sonraki bölüme geç
    ...
    tasks = db.query(DailyTask).filter( # daily tasks güncelleme
    ...
    if progress.subsection_completion == 3: # section tamamlama görevi
    ...
    return {'message': "Doğru cevap!", 'health_count': user.health_count,'progress_updated': True,'current_subsection': progress.current_subsection,'subsection_completion': progress.subsection_completion}