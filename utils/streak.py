...
def update_user_streak(db: Session, user_id: int, lesson_id: int): # kullanıcının seçtiği ders için streak durumunu kontrol eder
    try:
...
def update_all_users_streaks(): # tüm kullanıcıların streak durumunu günceller
    db = SessionLocal()
    try:
